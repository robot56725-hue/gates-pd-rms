...
-- set_config's 3rd arg is FALSE (session-level, not transaction-local)
-- specifically so this works as two independent auto-committed statements --
-- Neon's SQL Editor runs EXPLAIN on whatever you submit for its cost-estimate
-- panel, and EXPLAIN cannot be applied to an explicit BEGIN/COMMIT block.
SELECT set_config('app.actor_role', 'System_Admin', false);
